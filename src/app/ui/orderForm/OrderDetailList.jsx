...
export default function OrderDetailList() {
  ...
  function addQuantity(product) {
    const newList = productList.map((item, idx) =>
      idx === findId(product) ? { ...item, quantity: item.quantity + 1 } : item
    )
  ...
  return (
    <div className="flex gap-2 rounded-xl flex-col bg-neutral-100 dark:bg-neutral-900 p-2">
      ...
      <div className="flex justify-between px-2 items-center">
        ...
        <span className="text-xs font-semibold">Cantidades: {orderTotals.quantity}</span>
        ...
        productList.map(product =>
          <ProductCard
            ...
            <div
              ...
              <span className="flex justify-center items-center text-xs w-8 py-1">{product.quantity}</span>
              <QuantityButton
                icon="+"
