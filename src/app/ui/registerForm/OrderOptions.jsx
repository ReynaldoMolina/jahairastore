...
export function OrderOptions({ order }) {
  ...
  const balance = Math.round((formTotals.totalSell - order.TotalAbono) * 100) / 100;
    ...
    <FormOptionContainer>
      {(formTotals.totalSell - order.TotalAbono) > 0 && (
        <div className="flex gap-1 items-center">
          ...
          <FormOption label="100%" href={`/recibos/create?pedido=${order.Id}&cliente=${order.Id_cliente}&saldo=${balance}&abono=${balance}`}>
          </FormOption>
        </div>
      )}
      {order.TotalAbono > 0 &&
        <FormOption label="Recibos" href={`/recibos?query=${order.Id} ${order.NombreCliente}`}>
