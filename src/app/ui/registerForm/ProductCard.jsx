    ...
    >
      ...
      <CardInfo>
        ...
        <CardInfoDetail>
          ...
          {children}
          {showLeft && (
          <span className="text-xs min-w-18 text-left sm:text-right text-neutral-400">Quedan {product.Existencias}</span>
        )}
        </CardInfoDetail>
      </CardInfo>
      <div className="flex gap-2 flex-col">
        <span className={`text-xs font-bold min-w-19 pr-0.5 text-right ${price === 'compra' ? "text-red-400" : "text-green-400"}`}>{convert ? 'C$' : '$'} {subtotal.toFixed(2)}</span>
        {showProfit && (
