...
export function MenuItem({ option }) {
  ...
  return (
    <div key={option.id} className="flex md:flex-col items-center w-full gap-1">
      <Link
        href={option.url}
        className={`flex flex-col md:flex-row min-w-17 md:w-full justify-center md:justify-start items-center cursor-pointer rounded-lg md:rounded-md gap-1 md:gap-2 p-2 text-xs ${
          isActive
