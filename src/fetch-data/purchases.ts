  ...
  try {
    const data = await db
      .select({
        ...
        fecha: compras.fecha,
        total_compra: sql<number>`COALESCE(SUM(${compras_detalles.cantidad} * ${compras_detalles.precio_compra} * ${compras_detalles.cambio_dolar}), 0)`,
        total_gastos: sql<number>`COALESCE(SUM(${egresos.gasto} * ${egresos.cambio_dolar}), 0)`,
        ganancia: sql<number>`
        COALESCE(SUM(${compras_detalles.cantidad} * ${compras_detalles.precio_venta} * ${compras_detalles.cambio_dolar}), 0) - COALESCE(SUM(${compras_detalles.cantidad} * ${compras_detalles.precio_compra} * ${compras_detalles.cambio_dolar}), 0) - COALESCE(SUM(${egresos.gasto} * ${egresos.cambio_dolar}), 0)`,
      })
      .from(compras)
    ...
    const totalPages = await getPurchasesPages(filterBySearch, limit ?? 0);
...
async function getPurchasesPages(
